...
const MedidasList = ({ onEdit, refresh }) => {
  ...
  const fetchMedidas = async () => {
    try {
      const response = await obtenerListaMedidas(); // Fetch all brands from the API
  ...
  const toggleActiveStatus = async (id, currentStatus) => {
    try {
      const result = await actualizarMedida(id, { active: !currentStatus });
      if (result) {
        fetchMedidas();
      }
    } catch (error) {
  ...
  const deleteHeader = async (id) => {
    confirm({
      ...
      async onOk() {
        try {
          const result = await eliminarMedida(id);
          if (result.data) {
            setReload(!reload);
