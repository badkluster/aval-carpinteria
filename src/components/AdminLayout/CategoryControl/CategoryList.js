...
import CategoriasSection from "../../ComponentesHome/CategoriasSection";

const CategoryList = ({ onEdit, refresh }) => {
  ...
  const fetchCategorias = async () => {
    try {
      const response = await obtenerListaCategorias(); // Fetch all categorias from the API

      if (response.data?.length > 0) {
        setCategorias(response.data); // Set categorias if data exists
      }
    } catch (error) {
  ...
  const toggleActiveStatus = async (id, currentStatus) => {
    try {
      const result = await actualizarCategoria(id, { active: !currentStatus });
      if (result) {
        fetchCategorias();
      }
    } catch (error) {
        ...
        <>
                    ...
                    <TableRow key={category._id}>
                      ...
                      <TableCell>
                        <Switch
                          ...
                          onChange={() =>
                            toggleActiveStatus(category._id, category.active)
                          }
                        />
                      </TableCell>
                      <TableCell>
          ...
          {showPreview && (
            <Box mt={2}>
              ...
              <CategoriasSection
                categories={categorias.filter((header) => header.active)}
              />
            </Box>
          )}
        </>
