...
const HeaderList = ({ onEdit, refresh }) => {
  ...
  const fetchHeaders = async () => {
    try {
      const response = await obtenerListaHeaders();
  ...
  const toggleActiveStatus = async (id, currentStatus) => {
    try {
      const result = await actualizarHeader(id, { active: !currentStatus });
      if (result) {
        fetchHeaders();
      }
    } catch (error) {
  ...
  const deleteHeader = async (id) => {
    confirm({
      ...
      async onOk() {
        try {
          const result = await eliminarHeader(id);
          if (result.data) {
            fetchHeaders();
        ...
        <>
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell>Copete</TableCell>
                  <TableCell>Descripción</TableCell>
                  <TableCell>Titulo de botón</TableCell>
                  <TableCell>URL botón</TableCell>
                ...
                {headers
                  ...
                  .map((header) => (
                    <TableRow key={header._id}>
                      ...
                      <TableCell>{header.linkTitle}</TableCell>
          ...
          {showPreview && (
            <Box mt={2}>
              ...
              <Header slides={headers.filter((header) => header.active)} />
            </Box>
          )}
        </>
      )}
